...
(defun my/holiday-hong-kong-general-holidays-of-year-from-2011 (year)
  "Return a list of Hong Kong General Holidays in YEAR equal to or greater than 2011."
  (when (< year 2011)
    (error "Hong Kong General Holidays is only defined for year >= 2011: %d" year))
  (let* ((table (make-hash-table :test #'eql))
    ...
    (let* ((date `(1 1 ,year))
           (day-number (calendar-absolute-from-gregorian date)))
      (if (gethash day-number table)
          (let* ((day-number (1+ day-number))
                 (date (calendar-gregorian-from-absolute day-number)))
            (setq holidays (append holidays (list (list date "[香港公眾假期] 一月一日翌日")) nil))
            (puthash day-number t table))
        (setq holidays (append holidays (list (list date "[香港公眾假期] 一月一日")) nil))
        (puthash day-number t table)))
    ;; Cap. 149 General Holidays Ordinance Schedule (c) the Lunar New Year's day
    (let* ((date (my/calendar-chinese-month-day-of-year year 1 1))
           (day-number (calendar-absolute-from-gregorian date)))
      (if (gethash day-number table)
          (let* ((day-number (+ day-number 3))
                 (date (calendar-gregorian-from-absolute day-number)))
            (setq holidays (append holidays (list (list date "[香港公眾假期] 農曆年初四")) nil))
            (puthash day-number t table))
        (setq holidays (append holidays (list (list date "[香港公眾假期] 農曆年初一")) nil))
        (puthash day-number t table)))
    ;; Cap. 149 General Holidays Ordinance Schedule (d) the second day of Lunar New Year
    (let* ((date (my/calendar-chinese-month-day-of-year year 1 2))
           (day-number (calendar-absolute-from-gregorian date)))
      (if (gethash day-number table)
          (let* ((day-number (+ day-number 2))
                 (date (calendar-gregorian-from-absolute day-number)))
            (setq holidays (append holidays (list (list date "[香港公眾假期] 農曆年初四")) nil))
            (puthash day-number t table))
        (setq holidays (append holidays (list (list date "[香港公眾假期] 農曆年初二")) nil))
        (puthash day-number t table)))
    ;; Cap. 149 General Holidays Ordinance Schedule (e) the third day of Lunar New Year
    (let* ((date (my/calendar-chinese-month-day-of-year year 1 3))
           (day-number (calendar-absolute-from-gregorian date)))
      (if (gethash day-number table)
          (let* ((day-number (+ day-number 1))
                 (date (calendar-gregorian-from-absolute day-number)))
            (setq holidays (append holidays (list (list date "[香港公眾假期] 農曆年初四")) nil))
            (puthash day-number t table))
        (setq holidays (append holidays (list (list date "[香港公眾假期] 農曆年初三")) nil))
        (puthash day-number t table)))
    ;; Cap. 149 General Holidays Ordinance Schedule (f) Ching Ming Festival
    (let* ((date (car (my/solar-term "清明" year)))
           (day-number (calendar-absolute-from-gregorian date)))
      (if (gethash day-number table)
          (let* ((day-number (+ day-number 1))
                 (date (calendar-gregorian-from-absolute day-number)))
            (setq holidays (append holidays (list (list date "[香港公眾假期] 清明節翌日")) nil))
            (puthash day-number t table))
        (setq holidays (append holidays (list (list date "[香港公眾假期] 清明節")) nil))
        (puthash day-number t table)))
    ...
    (let* ((easter-abs (holiday-easter-etc-abs year)))
      ;; Good Friday
      (let* ((day-number (- easter-abs 2))
             (date (calendar-gregorian-from-absolute day-number)))
        (if (gethash day-number table)
            (let* ((day-number (+ day-number 4))
                   (date (calendar-gregorian-from-absolute day-number)))
              (setq holidays (append holidays (list (list date "[香港公眾假期] 復活節星期一翌日")) nil))
              (puthash day-number t table))
          (setq holidays (append holidays (list (list date "[香港公眾假期] 耶穌受難節")) nil))
          (puthash day-number t table)))
      ;; the day following Good Friday
      (let* ((day-number (- easter-abs 1))
             (date (calendar-gregorian-from-absolute day-number)))
        (if (gethash day-number table)
            (let* ((day-number (+ day-number 3))
                   (date (calendar-gregorian-from-absolute day-number)))
              (setq holidays (append holidays (list (list date "[香港公眾假期] 復活節星期一翌日")) nil))
              (puthash day-number t table))
          (setq holidays (append holidays (list (list date "[香港公眾假期] 耶穌受難節翌日")) nil))
          (puthash day-number t table)))
      ;; Easter Monday
      (let* ((day-number (+ easter-abs 1))
             (date (calendar-gregorian-from-absolute day-number)))
        (if (gethash day-number table)
            (let* ((day-number (+ day-number 1))
              ...
              (setq holidays (append holidays (list (list date "[香港公眾假期] 復活節星期一翌日")) nil))
              (puthash day-number t table))
          (setq holidays (append holidays (list (list date "[香港公眾假期] 復活節星期一")) nil))
          (puthash day-number t table))))
    ...
    (let* ((date `(5 1 ,year))
      ...
      (if (gethash day-number table)
          (let* ((day-number (1+ day-number))
                 (date (calendar-gregorian-from-absolute day-number)))
            (setq holidays (append holidays (list (list date "[香港公眾假期] 勞動節翌日")) nil))
            (puthash day-number t table))
        (setq holidays (append holidays (list (list date "[香港公眾假期] 勞動節")) nil))
        (puthash day-number t table)))
    ;; Cap. 149 General Holidays Ordinance Schedule (k) the birthday of the Buddha
    (let* ((date (my/calendar-chinese-month-day-of-year year 4 8))
           (day-number (calendar-absolute-from-gregorian date)))
      (if (gethash day-number table)
          (let* ((day-number (1+ day-number))
                 (date (calendar-gregorian-from-absolute day-number)))
            (setq holidays (append holidays (list (list date "[香港公眾假期] 佛誕翌日")) nil))
            (puthash day-number t table))
        (setq holidays (append holidays (list (list date "[香港公眾假期] 佛誕")) nil))
        (puthash day-number t table)))
    ;; Cap. 149 General Holidays Ordinance Schedule (l) Tuen Ng Festival
    (let* ((date (my/calendar-chinese-month-day-of-year year 5 5))
           (day-number (calendar-absolute-from-gregorian date)))
      (if (gethash day-number table)
          (let* ((day-number (1+ day-number))
                 (date (calendar-gregorian-from-absolute day-number)))
            (setq holidays (append holidays (list (list date "[香港公眾假期] 端午節翌日")) nil))
            (puthash day-number t table))
        (setq holidays (append holidays (list (list date "[香港公眾假期] 端午節")) nil))
        (puthash day-number t table)))
    ...
    (let* ((date `(7 1 ,year))
      ...
      (if (gethash day-number table)
          (let* ((day-number (1+ day-number))
                 (date (calendar-gregorian-from-absolute day-number)))
            (setq holidays (append holidays (list (list date "[香港公眾假期] 香港特別行政區成立紀念日翌日")) nil))
            (puthash day-number t table))
        (setq holidays (append holidays (list (list date "[香港公眾假期] 香港特別行政區成立紀念日")) nil))
        (puthash day-number t table)))
    ...
    (let* ((date `(10 1 ,year))
      ...
      (if (gethash day-number table)
          (let* ((day-number (1+ day-number))
                 (date (calendar-gregorian-from-absolute day-number)))
            (setq holidays (append holidays (list (list date "[香港公眾假期] 國慶日翌日")) nil))
            (puthash day-number t table))
        (setq holidays (append holidays (list (list date "[香港公眾假期] 國慶日")) nil))
        (puthash day-number t table)))
    ;; Cap. 149 General Holidays Ordinance Schedule (o) the day following the Chinese Mid-Autumn Festival
    (let* ((date (my/calendar-chinese-month-day-of-year year 8 16))
           (day-number (calendar-absolute-from-gregorian date)))
      (if (gethash day-number table)
          (let* ((day-number (1+ day-number))
                 (date (calendar-gregorian-from-absolute day-number)))
            (setq holidays (append holidays (list (list date "[香港公眾假期] 中秋節後第二日")) nil))
            (puthash day-number t table))
        (setq holidays (append holidays (list (list date "[香港公眾假期] 中秋節翌日")) nil))
        (puthash day-number t table)))
    ;; Cap. 149 General Holidays Ordinance Schedule (p) Chung Yeung Festival
    (let* ((date (my/calendar-chinese-month-day-of-year year 9 9))
           (day-number (calendar-absolute-from-gregorian date)))
      (if (gethash day-number table)
          (let* ((day-number (1+ day-number))
                 (date (calendar-gregorian-from-absolute day-number)))
            (setq holidays (append holidays (list (list date "[香港公眾假期] 重陽節翌日")) nil))
            (puthash day-number t table))
        (setq holidays (append holidays (list (list date "[香港公眾假期] 重陽節")) nil))
        (puthash day-number t table)))
    ...
    (let* ((date `(12 25 ,year))
      ...
      (if (gethash day-number table)
          (let* ((day-number (+ day-number 2))
                 (date (calendar-gregorian-from-absolute day-number)))
            (setq holidays (append holidays (list (list date "[香港公眾假期] 聖誕節後第二個周日")) nil))
            (puthash day-number t table))
        (setq holidays (append holidays (list (list date "[香港公眾假期] 聖誕節")) nil))
        (puthash day-number t table)))
    ...
    (let* ((date `(12 26 ,year))
      ...
      (if (gethash day-number table)
          (let* ((day-number (+ day-number 1))
            ...
            (setq holidays (append holidays (list (list date "[香港公眾假期] 聖誕節後第二個周日")) nil))
            (puthash day-number t table))
        (setq holidays (append holidays (list (list date "[香港公眾假期] 聖誕節後第一個周日")) nil))
        (puthash day-number t table)))
    ;; Special Holiday (3 September 2015) Ordinance
    (when (eql year 2015)
      (let* ((date '(9 3 2015))
             (day-number (calendar-absolute-from-gregorian date)))
        (setq holidays (append holidays (list (list date "[香港公眾假期] 抗日戰爭勝利七十周年紀念日")) nil))
        (puthash day-number t table)))
...
When STRICT is non-nil, error if holidays cannot be derived."
  (pcase-let* ((`(,_ ,y1 ,_ ,y2) (calendar-get-month-range)))
    (when (and strict (or (< y1 2011) (< y2 2011)))
      (error "Hong Kong General Holidays cannot be derived for %d or %d" y1 y2))
    (holiday-filter-visible-calendar
     (append
      (when (>= y1 2011)
        (my/holiday-hong-kong-general-holidays-of-year-from-2011 y1))
      (when (and (>= y2 2011) (/= y1 y2))
        (my/holiday-hong-kong-general-holidays-of-year-from-2011 y2))
      nil))))
