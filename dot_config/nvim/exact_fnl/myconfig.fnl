...
(set vim.opt.listchars {:tab ">_" :lead "." :trail "-" :nbsp "+"})
...
(create_user_command :HighlightGroupAtCursor
                     (fn []
...
(create_autocmd [:BufNewFile :BufRead]
                {:pattern :*.env
                 ...
                 :group (create_augroup :MyDotEnv {:clear true})})

(create_autocmd [:TextYankPost]
                {:pattern "*"
                 :callback (fn [] (vim.highlight.on_yank))
                 :group (create_augroup :MyYankHighlight {:clear true})})

(create_autocmd [:DiagnosticChanged]
                {:callback (fn [] (vim.diagnostic.setloclist {:open false}))
                 :group (create_augroup :MyDiagnostic {:clear true})})
